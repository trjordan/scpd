...
public class AuthWebServer {                            
	...
	// LAB_TODO: add HTML to render image here
	...
	String operation = postData.getValue("submit");
 	...
 	if (pathname.startsWith("/login") && (operation.equals("login"))) {
	    ...
	    osw.write ("HTTP/1.0 200 OK\n");      
	    osw.write ("\n\n<html><body>User '"+username+"' Registered!");
	...
	try {                                               
	    fr = new FileInputStream (checkPath(pathname));                 
	    c = new byte[fr.available()];
	    fr.read(c);                                  
	}                                                   
	catch (Exception e) {                               
	...
	osw.write("Content-Type: text/html\n\n");
	osw.flush();
	System.err.print(new String(c, "UTF-8"));
	os.write(c);
    }                                                       
